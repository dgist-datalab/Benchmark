...
if [ -z $DB_DIR ]; then
  echo "DB_DIR $DB_DIR is not defined"
#exit 0
fi

if [ -z $WAL_DIR ]; then
  echo "WAL_DIR $WAL_DIR is not defined"
#exit 0
fi
...
cache_size=${CACHE_SIZE:-$((4 * G))}
compression_max_dict_bytes=${COMPRESSION_MAX_DICT_BYTES:-0}
compression_type=${COMPRESSION_TYPE:-none}
duration=${DURATION:-0}

num_keys=${NUM_KEYS:-$((8 * Mi))}
#num_reads=${NUM_READS:--1}
#num_writes=${NUM_WRITES:--1}
num_reads=${NUM_READS:-$(($num_keys / $num_threads))}
num_writes=${NUM_WRITES:-$(($num_keys / $num_threads))}
...
#  echo "RW $num_reads random keys"
  echo "RW $num_keys random keys"
  out_name="benchmark_readrandomwriterandom.t${num_threads}.log"
  cmd="./db_bench --benchmarks=readrandomwriterandom \
       --use_existing_db=1 \
       $params_w \
       --threads=$num_threads \
	   --reads=$num_reads \
...
function run_readrandom_direct {
  echo "Reading $num_keys random keys"
  out_name="benchmark_readrandom_direct.t${num_threads}.log"
  cmd="./db_bench --benchmarks=readrandom \
       --use_existing_db=1 \
       $params_w \
       --threads=$num_threads \
...
function run_readseq {
  echo "Reading $num_keys sequential keys"
  out_name="benchmark_readseq.t${num_threads}.log"
  cmd="./db_bench --benchmarks=readseq \
       --use_existing_db=1 \
       $params_w \
       --threads=$num_threads \
	   --reads=$num_reads \
...
function run_readseq_direct {
  echo "Reading $num_keys sequential keys"
  out_name="benchmark_readseq_direct.t${num_threads}.log"
  cmd="./db_bench --benchmarks=readseq \
       --use_existing_db=1 \
       $params_w \
       --threads=$num_threads \
...
for job in ${jobs[@]}; do
  ...
  if [ $job = bulkload ]; then
    ...
    run_readseq
  elif [ $job = readrandom_direct ]; then
    run_readrandom_direct
  elif [ $job = readseq_direct ]; then
    run_readseq_direct
